...
def train_and_evaluate(data_dict, freeze_level=0, loss_fn='nb'):
    ...
    for idx, new_batch in enumerate(target_conditions):
        ...
        new_network = sn.operate(new_network,
        ...
        train_adata, valid_adata = sn.utils.train_test_split(batch_adata, 0.80)
        ...
        adata_vis.uns[f'{cell_type_key}_colors'] = prev_cell_type_colors + batch_adata.uns[
            f'{cell_type_key}_colors'].tolist()

        encoder_labels, _ = sn.utils.label_encoder(adata_vis, label_encoder=new_network.condition_encoder,
                                                   condition_key=batch_key)

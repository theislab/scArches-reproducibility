...
source("/home/marco/Documents/git_repos/thesis_marco/metrics/knit_table.R") # You will need to have in the same folder knit_table.R and this plotSingleAtlas.R
...
plotSingleAtlas <- function(csv_file_path, outdir="/home/marco/Documents/metric_plots"){
  ...
  for (dt.sc in data.scenarios){
    ...
    g <- scIB_knit_table(data = metrics_tab, column_info = column_info, row_info = row_info, palettes = palettes, plot_time=TRUE)  
    now <- Sys.time()
